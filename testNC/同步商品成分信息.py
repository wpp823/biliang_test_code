...
def get_product_id_list(status, page):
...
def sync_product_component_data(product_id):
    sync_url = f'http://{server_url}:18851/in/mnc/api/v1/product/component'
    data = {
...
if __name__ == "__main__":
    ...
    for status in SPU_STATUS:
        page_num = 1
        while True:
            res_data = get_product_id_list(status=status, page=page_num)
            if res_data:
                ...
                for item in res_data:
                    try:
                        ...
                        sync_product_component_data(product_id=product_id)
