...
from mongo_db.model.product import ProductModel
...
async def mouse_slide(page=None):
    ...
    try:
        ...
        await page.hover('#nc_1__scale_text > span')  # 不同场景的验证码模块能名字不同。//*[@id="nc_1__scale_text"]/span
...
if __name__ == "__main__":
    ...
    for product in product_ext_urls:
            ...
            try:
                ...
                for i in range(3):
                    ...
                    if ext_page_info:
                        ...
                        log.info(f"product_id:{product_id},sku_id:{sku_id},ext_url:{ext_url}  price:{price}")
                        break
                    else:
                        time.sleep(random.randint(1, 4))
                        ext_price_info = asyncio.get_event_loop().run_until_complete(price_tool.login_taobao())
                        log.info("[sync_third_party_product_price][login_taobao][ext_price_info:{}]".format(ext_price_info))
            except:
