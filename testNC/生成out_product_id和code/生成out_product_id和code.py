...
file_path = './正式服expect_data_0315.xls'
...
out_product_dict = {
    product_id: 'P_' + '_'.join([yinjie(item) for item in data[data.product_id == product_id].brand.to_list()[0].split(',')])+f"_{now_at}" + f'_{index}'
...
data.insert(loc=1,column="spu_code", value=new_out_product_ids)
data.insert(loc=1, column="code", value=new_codes)

data.to_excel("new_spu_code_sku_code.xls")
pass
